...
*/
			...
			//				pthread_create(send+i, NULL, send_file, (void *)(client_info + i));
...
//	send_msg(&msg_info, first_client_info);
	...
	while(1)
	{
		...
		wfd = open("Totallog.txt", O_RDWR | O_CREAT | O_APPEND);
		if(wfd < 0)
        	error_handling("write file open() error!");

		sprintf(strBuff,"msg : [%s->%s] %s",msg_info.from,msg_info.to,pArray[1]);
		
		sprintf(totallog,"%d.%d.%d %d:%d [%s->%s] %s",t->tm_year +1900, t->tm_mon +1, t->tm_mday, t->tm_hour, t->tm_min, msg_info.from,msg_info.to,pArray[1]);
